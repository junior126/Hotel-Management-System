...
$result = mysql_query("SELECT * FROM room where max_adult >='$adults' and max_child>='$child'");

while($row = mysql_fetch_array($result))
  {
  $a=$row['room_id'];
  $query = mysql_query("SELECT sum(qty_reserve) FROM roominventory where arrival >= '$arrival' and departure <= '$departure' and room_id='$a'");
...
$angavil = $row['qty'] - $inogbuwin ;  
  ...
  echo '<div style="height: 117px;">';
	  ...
	  echo '<span class="style5">'.'Available Rooms: '.$angavil.'</span>';
	  if ($angavil > 0){
				...
				if ($angavil <= 0){
				echo '<span class="style5">'.'NO ROOM IS AVAILABLE'.'</span>';
				}	
	  echo '<br>';		
...
<label style="margin-left: 16px;">Check In Date : <?php echo $arrival; ?></label><br />
